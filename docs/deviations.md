...
## Synthetic Options Chains (Phase 2)

The 20 options strategies shipping in Phase 2 (Session 2F) back-test
against **synthetic** `OptionChain` snapshots generated by
`alphakit.data.options.synthetic.SyntheticOptionsFeed` — not real
market data. The feed prices a 9-strike × 11-14-expiry grid via
Black-Scholes using trailing-realized-volatility (30/60/90-day) as a
flat IV, as documented in [ADR-005](adr/005-synthetic-options-chain.md)
and [docs/feeds/synthetic-options.md](feeds/synthetic-options.md).
The Polygon adapter (ADR-004) ships as a registered placeholder in
this phase; its `fetch_chain` raises `PolygonNotConfiguredError`
without an API key and `NotImplementedError` with one.

Structural limitations carry through to every options strategy's
back-test: there is **no IV skew** (flat vol across strikes), **no
bid-ask spread** (bid = ask = mid = BS-price), **no realistic volume
or open interest**, and **greeks are BS-computed rather than
market-implied**. Strategies that monetise skew (put-skew premium,
risk reversals, dispersion proxies), microstructure (bid-ask
capture, liquidity timing), or event-driven vol structure (earnings
vol crush) therefore back-test against approximations rather than
markets. Every Phase 2 options strategy's `paper.md` includes a
"Data Fidelity" note calling out the specific mechanism it is
substituting for.

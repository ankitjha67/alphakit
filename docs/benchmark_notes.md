...
# Phase 2 real-feed cluster — 5 FRED-gated regime strategies (v0.2.1)

Generated by `scripts/cluster_analysis.py --feed real` (Session 2I). Pairwise
Pearson correlation of the 5 FRED-gated regime-state macro strategies'
equity-curve returns, computed on **real yfinance + FRED data** via the
multi-feed `BenchmarkRunner(strict_feed=True)` — 5,282 aligned bars (2005-2025).

**Incomparability note (load-bearing).** This real-feed 5×5 is **not** pooled
with the 47-strategy synthetic-fixture matrix above. A real-feed ρ and a
synthetic-fixture ρ are computed on different underlying price paths; only the
5 regime strategies (whose signals are degenerate on generic fixtures) are
re-measured here. This supersedes, for these 5 strategies only, the
"regime-degenerate on fixtures — prediction authoritative" caveat above.

## Real-feed 5×5 ρ matrix

| | recession | growth_inf | yield_curve | fed_policy | inflation |
|---|---|---|---|---|---|
| recession | 1.000 | 0.529 | 0.495 | 0.452 | 0.478 |
| growth_inf | 0.529 | 1.000 | 0.478 | 0.540 | 0.779 |
| yield_curve | 0.495 | 0.478 | 1.000 | 0.794 | 0.672 |
| fed_policy | 0.452 | 0.540 | 0.794 | 1.000 | 0.637 |
| inflation | 0.478 | 0.779 | 0.672 | 0.637 | 1.000 |

## Predicted (Session 2G) vs actual ρ

| actual | predicted | in range | pair |
|---|---|---|---|
| +0.794 | 0.40–0.60 | **OUT (high)** | yield_curve_regime_allocation ↔ fed_policy_tilt |
| +0.779 | 0.40–0.60 | **OUT (high)** | growth_inflation_regime_rotation ↔ inflation_regime_allocation |
| +0.672 | 0.30–0.50 | **OUT (high)** | yield_curve_regime_allocation ↔ inflation_regime_allocation |
| +0.637 | 0.30–0.50 | **OUT (high)** | fed_policy_tilt ↔ inflation_regime_allocation |
| +0.540 | 0.40–0.60 | OK | growth_inflation_regime_rotation ↔ fed_policy_tilt |
| +0.529 | 0.40–0.60 | OK | recession_probability_rotation ↔ growth_inflation_regime_rotation |
| +0.495 | 0.50–0.70 | **OUT (low)** | recession_probability_rotation ↔ yield_curve_regime_allocation |
| +0.478 | 0.40–0.60 | OK | growth_inflation_regime_rotation ↔ yield_curve_regime_allocation |
| +0.478 | 0.30–0.50 | OK | recession_probability_rotation ↔ inflation_regime_allocation |
| +0.452 | 0.40–0.60 | OK | recession_probability_rotation ↔ fed_policy_tilt |

**5/10 within the Session 2G predicted range.** Mean |ρ| = 0.585; max ρ = +0.794;
no pair breaches the dedup-review bar (ρ > 0.95).

## Economic interpretation

Real macro regime strategies share **more** co-movement than the
synthetic-fixture analysis predicted: 4 of the 5 out-of-range pairs sit *above*
their predicted band, and the higher correlations cluster around a few macro
drivers:

1. **Fed policy permeates everything.** Yield curves, inflation expectations and
   the policy tilt all respond to the same FOMC actions —
   `yield_curve_regime_allocation ↔ fed_policy_tilt` (0.794) is the strongest
   pair.
2. **Inflation drives multiple signals.** `growth_inflation_regime_rotation` and
   `inflation_regime_allocation` share the CPI component (0.779).
3. **Recession indicators are upstream** of both yield-curve shape and Fed
   response, seeding second-order correlation across the complex.

The one pair Session 2G singled out as *deliberate redundancy* —
`recession_probability_rotation ↔ yield_curve_regime_allocation` — actually came
in at **0.495, fractionally below** its 0.50–0.70 band (essentially as
predicted). So the redundancy in this group is **not concentrated in the flagged
pair**; it is a **diffuse macro-wide common factor** running through the
inflation- and fed-policy-linked cross-pairs that Session 2G predicted would be
*lower*. The synthetic predictions were directionally right about *which* pair
would correlate but wrong about *where* the macro complex's common factor would
show up.

**Practical implication.** These 5 strategies offer **less diversification
benefit** than the synthetic predictions implied — for portfolio construction,
treat them closer to a single macro-regime-detection bet than five independent
ones. The Session 2G "deliberate-redundancy pair" framing should be widened: the
common factor is the macro complex itself, not one pair. None of this is a
defect — it is exactly the truth a real-feed pass exists to surface, and all
pairs remain well below the ρ > 0.95 dedup bar.
